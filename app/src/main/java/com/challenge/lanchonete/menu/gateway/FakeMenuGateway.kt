...
class FakeMenuGateway : MenuGateway {
    override fun getAllSandwiches(): List<Sandwich> {
        ...
        val xBacon = Sandwich(1, "X-Bacon", listOf(bacon, meatHamburger, cheese))
        xBacon.calculatePrice()
        val xBurger = Sandwich(2, "X-Burger", listOf(meatHamburger, cheese))
        xBurger.calculatePrice()
        val xEgg = Sandwich(3, "X-Egg", listOf(egg, meatHamburger, cheese))
        xEgg.calculatePrice()
        val xEggBacon = Sandwich(4, "X-Egg Bacon", listOf(egg, bacon, meatHamburger, cheese))
